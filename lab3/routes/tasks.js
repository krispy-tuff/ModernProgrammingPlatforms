...
router.post('/tasks', upload.array('attachments', 10), [
    body('title').trim().isLength({ min: 1 }).withMessage('Название задачи обязательно'),
    body('status').isIn(['pending', 'in_progress', 'completed', 'cancelled']).withMessage('Неверный статус'),
...
router.post('/tasks/:id/attachments', upload.array('attachments', 10), (req, res) => {
    const tasks = readTasks();
    const taskIndex = tasks.findIndex(t => t.id === req.params.id);
...
router.delete('/attachments/:id', (req, res) => {
    ...
    taskOfAttachment.attachments = taskOfAttachment.attachments.filter(a => a.id !== req.params.id);
