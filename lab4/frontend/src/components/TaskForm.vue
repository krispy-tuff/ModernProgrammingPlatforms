...
const visibleAttachments = computed(() => {
  ...
  return form.value.attachments.filter((att) => !attachmentsToDelete.value.includes(att.id))
})
      ...
      <div class="file-uploader">
        ...
        <ul class="file-list">
          <li v-for="file in visibleAttachments" :key="file.id">
            ...
            <button
              @click.stop.prevent="stageAttachmentForDeletion(file.id)"
              type="button"
              class="btn-remove"
            >
              &times;
          ...
          <li v-for="(file, index) in newlyAddedFiles" :key="index">
            {{ file.name }}
            <button @click="removeNewFile(index)" type="button" class="btn-remove">&times;</button>
          </li>
        </ul>
      </div>
    </div>
